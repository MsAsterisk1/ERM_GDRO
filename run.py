...
parser.add_argument('--trials', default=5, help='The number of trials to run, defaults to 5')
parser.add_argument('--test_name', default='test', help='The name of the test to run (defaults to "test"), the file will be saved at ./test_results/[name].csv')
...
parser.add_argument('--verbose', action='store_true', help='Print the progress and epoch results')
parser.add_argument('--cris_prop', default=0.7, help='The proportion of data to be dedicated to training the featurizer when using CRIS, defaults to 0.7 (ignored if not using CRIS)')
parser.add_argument('--subclass_labels', action='store_true', help='Train the model using the subclass labels (evalutaion is still done using the superclass labels, but is done with respect to each subclass)')
parser.add_argument('--val', default=None, help='If equal to 0, best overall validation accuracy is tracked and printed at the end of each trial. If another integer, best worst-group accuracy is used instead. If left empty, validation accuracy is not tracked (default)')
